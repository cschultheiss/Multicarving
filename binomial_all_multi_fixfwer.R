...
for (frac in frac_vec) {
  ...
  # res<-foreach(gu=1:nsim,.combine = rbind) %do%{
    ...
    mcrtry <- tryCatch_W_E(multi.carve(x, y, B = B, fraction = frac, model.selector = lasso.firstqcoef, classical.fit = glm.pval.pseudo,
                                       parallel = FALSE, ncores = getOption("mc.cores", 2L), gamma = 1,
                                       args.model.selector = list(standardize = FALSE, q = 16, intercept = TRUE, tol.beta = 0, family="binomial"),
                                       args.classical.fit =list(), repeat.max = 20, verbose = FALSE, FWER=FALSE, split_pval = TRUE,
                                       return.selmodels = TRUE, return.nonaggr = TRUE, 
                                       args.lasso_inference  =list(family = "binomial", selected = TRUE)), 0)

    c100try <- tryCatch_W_E(carve100(x, y, model.selector = lasso.firstqcoef,
                                     args.model.selector = list(standardize = FALSE, q = 16, intercept = TRUE, tol.beta = 1e-5, family = "binomial"),
                                     repeat.max = 20, verbose = FALSE, FWER=FALSE, return.selmodels = TRUE, return.nonaggr = TRUE,
                                     args.lasso_inference = list(family = "binomial")), 0)
  ...
  for (B in B_vec) {
    ...
    simulation <- list("results" = subres, "exceptions" = expmatr, "B" = B, "split" = frac,
                       "nsim" = nsim, "seed" = rseed, "All used B" = B_vec)
